...
class System:
    ...
    def __init__(self):
        ...
        self.__DATE_FORMAT = f"%Y{self.__DATE_SEPARATOR}%m{self.__DATE_SEPARATOR}%d"
        self.__DATE_TIME_FORMAT = f"{self.__DATE_FORMAT} {self.__TIME_FORMAT}"

    def get_time(self):
        return datetime.datetime.now(self.__DATE_TIME_REFERENCE).astimezone().strftime(self.__TIME_FORMAT).split(self.__TIME_SEPARATOR)

    def get_time_specific_region(self, region: str):
        try:
            return datetime.datetime.now(pytz.timezone(f"Africa/{region}")).strftime(self.__TIME_FORMAT).split(self.__TIME_SEPARATOR)
    ...
    def get_date(self):
        return datetime.datetime.now(self.__DATE_TIME_REFERENCE).astimezone().strftime(self.__DATE_FORMAT).split(self.__DATE_SEPARATOR)
